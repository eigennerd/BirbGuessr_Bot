...
def send_welcome(message):
    ...
    bot.reply_to(message,
                ...
                parse_mode='html')
    logger.info(f"{base64.b64encode(' '.join([message.from_user.first_name, message.from_user.last_name, message.from_id]).encode())} said hi on {datetime.now()}")
...
def echo_all(message):
    ...
    if message.content_type=='voice':
        try:
            if message.voice.duration < 5:  ## check for audio length
                bot.reply_to(message,
                             ...
                             , parse_mode='markdown')
                logger.warning(f"{base64.b64encode(' '.join([message.from_user.first_name, message.from_user.last_name, message.from_id]).encode())} sent an incomplete voice on {datetime.now()}")
            else:
                ...
                bot.reply_to(message,
                             ...
                             ,parse_mode='markdown')
                logger.info(f"{base64.b64encode(' '.join([message.from_user.first_name, message.from_user.last_name, message.from_id]).encode())} sent a complete voice on {datetime.now()}. Top accuracy was: {bird_proba}. Bird guessed was: {sci_name}")
        except Exception as e:
            bot.reply_to(message,
                         f"{text[lang]['hey']}, {message.from_user.first_name}. {text[lang]['sorry_wrong']} "
                         , parse_mode='markdown')
            logger.error(
                f"{base64.b64encode(' '.join([message.from_user.first_name, message.from_user.last_name, message.from_id]).encode())} encountered a voice error on {datetime.now()}. Error message: {e}")
        ...
        try:
            if message.audio.duration < 5: ## check for audio length
                bot.reply_to(message,
                             f"{text[lang]['hey']}, {message.from_user.first_name}. {text[lang]['i_require_5s']}"
                             , parse_mode='markdown')
                logger.warning(f"{base64.b64encode(' '.join([message.from_user.first_name, message.from_user.last_name, message.from_id]).encode())} sent an incomplete audio on {datetime.now()}")
            elif message.audio.duration >180:
                bot.reply_to(message,
                             f"{text[lang]['hey']}, {message.from_user.first_name}. {text[lang]['long_audio']}"
                             , parse_mode='markdown')
                logger.warning(f"{base64.b64encode(' '.join([message.from_user.first_name, message.from_user.last_name, message.from_id]).encode())} sent a too large audio file on {datetime.now()}")
            else:
                ...
                bot.reply_to(message,
                             ...
                             ,parse_mode='markdown')
                logger.info(
                    f"{base64.b64encode(' '.join([message.from_user.first_name, message.from_user.last_name, message.from_id]).encode())} sent a complete voice on {datetime.now()}. Top accuracy was: {bird_proba}")
        except Exception as e:
            bot.reply_to(message,
                         f"{text[lang]['hey']}, {message.from_user.first_name}. {text[lang]['sorry_wrong']}"
                         , parse_mode='markdown')
        ...
        bot.reply_to(message,
                     {text[lang]['try_again']})
        logger.warning(
            f"{base64.b64encode(' '.join([message.from_user.first_name, message.from_user.last_name, message.from_id]).encode())} sent gibberish on {datetime.now()}.")
